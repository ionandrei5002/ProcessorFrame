...
public:
    explicit FreeListPool(uint64_t size)
	{
        ...
        std::cout << "pool head at " << reinterpret_cast<void*>(head) << std::endl;

		cursor = head;
		tail = head + size;

        std::cout << "pool tail at " << reinterpret_cast<void*>(tail) << std::endl;
	...
	inline char* allocate(std::size_t size) override
	{
		...
		else
		{
			...
			{
				pointer = cursor;
                *reinterpret_cast<uint64_t*>(pointer) = total_size;
